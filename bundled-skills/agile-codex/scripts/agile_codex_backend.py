...
def read_jsonl_tail_objects(path: Path, limit: int | None = 32):
    if not path.exists():
        return []
    objects = []
    try:
        ...
        return []
    selected_lines = lines if limit is None or limit <= 0 else lines[-limit:]
...
def jsonl_state_hints(path: Path):
    objects = read_jsonl_tail_objects(path, limit=None)
    if not objects:
        return {
            "last_event_type": "",
            "last_payload_type": "",
    ...
    last_obj = objects[-1]
    payload = last_obj.get("payload") if isinstance(last_obj.get("payload"), dict) else {}
    return {
        "last_event_type": last_obj.get("type", ""),
        "last_payload_type": payload.get("type", "") if isinstance(payload, dict) else "",
        "task_complete_event": any(
            obj.get("type") == "event_msg"
            and isinstance(obj.get("payload"), dict)
            and obj["payload"].get("type") == "task_complete"
            for obj in objects
        ),
    }
